...
const optionName = 'nuxtPrecompress'

module.exports = function (moduleOptions) {
  const consola = require('consola')
  const userOptions = this.options[optionName] || moduleOptions
  const options = {
  ...
  this.extendBuild((config, { isDev }) => {
    if (isDev) {
      if (report) {
        consola.info('Skip activation of nuxt-precompress module in Dev mode')
      }
      return
    }
